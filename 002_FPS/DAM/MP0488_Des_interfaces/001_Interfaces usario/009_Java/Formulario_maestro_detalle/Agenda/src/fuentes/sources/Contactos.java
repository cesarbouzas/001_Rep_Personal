...
public class Contactos extends JPanel {
    ...
    private void deleteDetailButtonActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_deleteDetailButtonActionPerformed
        ...
        fuentes.sources.Contactos_1 C = list.get(masterTable.convertRowIndexToModel(index));
        Collection<fuentes.sources.Correos> cs = C.getCorreosList();
        int[] selected = detailTable.getSelectedRows();
        List<fuentes.sources.Correos> toRemove = new ArrayList<fuentes.sources.Correos>(selected.length);
        for (int idx = 0; idx < selected.length; idx++) {
            selected[idx] = detailTable.convertRowIndexToModel(selected[idx]);
            int count = 0;
            Iterator<fuentes.sources.Correos> iter = cs.iterator();
            while (count++ < selected[idx]) {
                iter.next();
            }
            fuentes.sources.Correos c = iter.next();
            toRemove.add(c);
    ...
    private void newDetailButtonActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_newDetailButtonActionPerformed
        ...
        Collection<fuentes.sources.Correos> cs = C.getCorreosList();
        if (cs == null) {
            cs = new LinkedList<fuentes.sources.Correos>();
            C.setCorreosList((List) cs);
        }
        fuentes.sources.Correos c = new fuentes.sources.Correos();
    ...
    private void deleteButtonActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_deleteButtonActionPerformed
        ...
        List<fuentes.sources.Contactos_1> toRemove = new ArrayList<fuentes.sources.Contactos_1>(selected.length);
        for (int idx = 0; idx < selected.length; idx++) {
            fuentes.sources.Contactos_1 C = list.get(masterTable.convertRowIndexToModel(selected[idx]));
            toRemove.add(C);
            entityManager.remove(C);
        }
    ...
    private void newButtonActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_newButtonActionPerformed
        fuentes.sources.Contactos_1 C = new fuentes.sources.Contactos_1();
        entityManager.persist(C);
        list.add(C);
        int row = list.size() - 1;
    ...
    private void saveButtonActionPerformed(java.awt.event.ActionEvent evt) {//GEN-FIRST:event_saveButtonActionPerformed
        try {
            ...
            rex.printStackTrace();
            entityManager.getTransaction().begin();
            List<fuentes.sources.Contactos_1> merged = new ArrayList<fuentes.sources.Contactos_1>(list.size());
            for (fuentes.sources.Contactos_1 C : list) {
    ...
    private java.util.List<fuentes.sources.Contactos_1> list;
